...
describe('Global Settings API', () => {
  ...
  it('should return defaults for new user', async () => {
    ...
    expect(res.body.settingsVisibility.fontSize).toBe(true);
  });
  ...
  it('should update fontMin and fontMax', async () => {
    ...
    expect(res.body.fontMin).toBe(10);
    expect(res.body.fontMax).toBe(30);
  });
  ...
  it('should update settings visibility', async () => {
    ...
    expect(res.body.settingsVisibility.sound).toBe(false);
    expect(res.body.settingsVisibility.fontSize).toBe(true);
  });
