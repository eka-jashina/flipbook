...
| Фаза 3 | ✅ Готово | Интеграция фронтенда с API |
| Фаза 4 | ⏳ Следующая | Надёжность и UX (offline, sync) |
...
**Цель:** Надёжная работа в реальных условиях (сбои сети, медленные соединения, офлайн)

**Принятые архитектурные решения:**

- **Sync queue** — в памяти (без IndexedDB). Причина: основной sync — прогресс чтения (debounce 5 сек + sendBeacon при закрытии). Потеря очереди при перезагрузке — потеря 2–3 страниц прогресса, не критично. IndexedDB добавляет async-сложность ради маловероятного кейса. При открытии книги GET /progress всегда подтягивает последнее сохранённое.
- **Конфликты прогресса** — Last-Write-Wins через `updatedAt` (уже есть в Prisma-модели ReadingProgress, `@updatedAt`). Без диалогов: прогресс чтения — низкоценный конфликт, диалог прерывает чтение. Два устройства одновременно — экзотика для читалки.
- **Оптимистичные обновления** — выборочно: прогресс/настройки/reorder — оптимистично (уже частично так); удаление/создание книги — ждём сервер (необратимо, нужен серверный ID). Undo не нужен — для удаления уже есть confirm().
- **Service Worker** — не трогаем. SW кеширует GET-статику (работает хорошо), retry мутаций — в ApiClient. Background Sync API — только Chrome. In-app retry предсказуемее и кроссбраузерный.
- **E2E тесты** — mock API через Playwright `page.route()`, без реального бэкенда. Серверные API покрыты supertest. Цель E2E здесь — поведение клиента при сбоях. Docker Compose в CI дорог (+30–60 сек). Реальный бэкенд — Фаза 5.

**Задачи:**

32. **Retry в ApiClient** (`_fetchWithRetry`) — exponential backoff из ContentLoader (~40 строк). Не retry 4xx, retry 5xx и network errors. Единая очередь с типизацией операции полем, не классом.
33. **Error handling в ServerAdminConfigStore** — try/catch + toast в каждом методе (~100 строк). Сейчас ошибки не ловятся в ~95% методов, exceptions ломают состояние admin-панели.
34. **SyncIndicator в reader** — новый компонент, 3 состояния: synced (скрыт/точка), syncing («Сохранение...»), error («Не сохранено», красный). Размещение рядом с navigation pod. ~80 строк JS + CSS.
35. **Error state в admin save indicator** — расширить существующий `#saveIndicator` (admin/index.js:340): добавить `save-indicator--error` + текст «Ошибка сохранения». ~20 строк.
36. **Optimistic reorder в BookshelfScreen** — drag-and-drop обновляет UI мгновенно, при ошибке API откатывает порядок. ~30 строк.
37. **E2E тесты: offline/error сценарии** — 3–5 тестов через Playwright route interception: offline→online, sync failure→retry→success, 401→AuthModal, медленный ответ→indicator.

### Фаза 5: Production

**Цель:** Готовность к деплою

38. CI/CD: тесты + деплой сервера
39. Мониторинг и логирование (pino structured logs)
40. Документация API (Swagger/OpenAPI)
41. HTTPS, домен, CDN для статики
42. **(По необходимости):** Redis для сессий (connect-redis)

### Фаза 6: Фотоальбом — фундамент

**Цель:** CRUD альбомов с S3-хранением фотографий

43. Схема БД: albums, album_pages, album_photos (Prisma миграция)
44. Image pipeline (sharp): resize, thumbnail, EXIF
45. Albums CRUD API
46. Photo upload: single + batch (до 50)
47. Slug generation (транслитерация + nanoid)
48. AlbumManager.js → S3 URL вместо base64

### Фаза 7: Фотоальбом — публичный доступ

**Цель:** Альбомы доступны по ссылке

49. Public routes: GET /api/public/album/:slug
50. Пароль на альбом (bcrypt + сессия)
51. Публичный viewer
52. Lightbox, ambient-звуки, текстуры
53. SEO: Open Graph, Twitter Card

### Фаза 8: Биллинг

**Цель:** Монетизация через Stripe

54. Схема БД: subscriptions
55. Stripe: Checkout, Portal, Webhooks
56. planLimits middleware
57. Billing UI + usage
...
58. album_views, user_branding (Prisma миграция)
59. Analytics API + дашборд (Chart.js)
60. White-label брендинг
61. Водяной знак (sharp overlay)
62. Embed-код (iframe)
...
63. Шаблоны тематик (свадьба, newborn, travel)
64. Автораскладка при batch upload
65. Кадрирование на клиенте
66. Реферальная программа
67. Мобильный редактор
...
| Фронтенд ApiClient | Один класс `js/utils/ApiClient.js` | ✅ Реализовано |
| Auth UI | Модалка в index.html, email/password | ✅ Реализовано |
| Admin store | Адаптер `ServerAdminConfigStore.js` | ✅ Реализовано |
| Миграция localStorage | При первом логине → `POST /api/import` → удалить | ✅ Реализовано |
| Контент глав | Через API эндпоинт (не S3) | ✅ Реализовано |
| Offline | Memory queue + retry в ApiClient, LWW для прогресса, без IndexedDB/SW | 📋 Фаза 4 |
